...
function pairCandidate(person, peopleById, assigned) {
  const candidates = [];
  if (person.currentPartnerId) candidates.push(person.currentPartnerId);
  person.partnerIds.forEach((partnerId) => {
    const partner = peopleById.get(partnerId);
    if (partner?.currentPartnerId === person.id) candidates.push(partnerId);
  });
  if (person.partnerIds.length === 1) {
    const partner = peopleById.get(person.partnerIds[0]);
    const isFormer = person.formerPartnerIds.includes(partner?.id) || partner?.formerPartnerIds.includes(person.id);
    if (partner?.partnerIds.length === 1 && !isFormer) candidates.push(partner.id);
  }
  return [...new Set(candidates)]
    .map((id) => peopleById.get(id))
    .find((partner) => partner && partner.generation === person.generation && !assigned.has(partner.id)) || null;
}
...
function createUnits(people) {
  ...
  people.forEach((person) => {
    ...
    const partner = pairCandidate(person, peopleById, assigned);
    const members = partner ? [person, partner] : [person];
    members.forEach((member) => assigned.add(member.id));
    const primaryParents = members.find((member) => member.parents.length)?.parents || [];
    units.push({
...
export function buildFamilyLayout(people) {
  if (!people.length) {
    return { units: [], cards: [], generations: [], width: MIN_STAGE_WIDTH, height: EMPTY_STAGE_HEIGHT };
  }
